...
m = Mesh(nproc_xi=16, chunk=1, ngll=5)
...
for i in range(m.nproc_xi):
    for j in range(m.nproc_xi):
        ...
        # Corner nodes:
        for rz in range(2):
            elem.cnodes_xyz.append(map_csb_to_xyz(xi2, eta1, r[rz], m.chunk))
            elem.cnodes_xyz.append(map_csb_to_xyz(xi2, eta2, r[rz], m.chunk))
            elem.cnodes_xyz.append(map_csb_to_xyz(xi1, eta2, r[rz], m.chunk))
            elem.cnodes_xyz.append(map_csb_to_xyz(xi1, eta1, r[rz], m.chunk))
